...
export function TransactionConfirmModal({
  ...
  const handleApproval = async () => {
    ...
    try {
      ...
      const spenderAddress = wallet.accounts[0].address; // replace with actual spender
      ...
      const tx = await usdcContract.approve(
        spenderAddress, // the bundler address
        ethers.parseUnits("100", 6) // example: approve 100 USDC (6 decimals)
      );

      console.log("Waiting for tx:", tx.hash);

      // This triggers wallet confirm modal and waits for it to be mined
      await tx.wait();
                  ...
                  <Button
                    onClick={async () => {
                      ...
                      const quoteReq = buildNewQuoteRequest({
                        ...
                        symbol: "USDC",
                        side: "1",
                        amount: transactions?.[0].amount.toString() || "0",
